...
export default function BlogPostContent({ slug }: { slug: string }) {
  ...
  if (loading) {
    return <div className="py-24 flex justify-center"><div className="animate-spin w-8 h-8 border-2 border-indigo-600 border-t-transparent rounded-full"></div></div>;
  }
  ...
  if (!post) {
    return (
      <div className="py-24 text-center">
        ...
        <Link href="/blog" className="text-indigo-600 hover:underline">{t("backToBlog")}</Link>
      </div>
    );
  }

  const title = locale === "ua" ? post.title_ua : post.title_en;
  const content = locale === "ua" ? (post.content_ua || post.content_en) : post.content_en;

  return (
    <div className="py-16 bg-white">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <Link href="/blog" className="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-indigo-600 mb-8 transition-colors">
          <ArrowLeft className="w-4 h-4" /> {t("backToBlog")}
        </Link>

        <article>
          <header className="mb-10">
            <h1 className="text-4xl lg:text-5xl font-bold text-slate-900 mb-6 leading-tight">{title}</h1>
        ...
        <div className="mt-16 pt-8 border-t border-slate-100 text-center">
          <p className="text-slate-600 mb-4">Ready to grow your business with Scalaro?</p>
          <a href="mailto:hello@scalaro.io" className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-indigo-600 to-blue-500 text-white font-semibold rounded-xl hover:from-indigo-700 hover:to-blue-600 transition-all text-sm">
            Book a Free Demo
          </a>
        </div>
      </div>
